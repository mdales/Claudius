...
let draw_background s fb =
  ...
  let horizon_height = (h / 3) in
    Framebuffer.filled_rect 0 0 w horizon_height 12 fb;
    Framebuffer.filled_rect 0 horizon_height 640 (h  - horizon_height) 8 fb
...
let draw_tree x y fb =
  ...
  Framebuffer.filled_rect (x - trunk_radius) (y + pot_height) (trunk_radius * 2) (trunk_height) 3 fb;
...
let tick t s fb _inputs =
  let _, h = Screen.dimensions s in
  draw_background s fb;
  draw_snow t 42 1.0 fb;
  draw_tree (h / 2) 60 fb;
  draw_snow t 62 2.0 fb;
  fb
