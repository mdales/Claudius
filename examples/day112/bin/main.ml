...
let generate_tile ft w h base_col over_col : Primitives.t list =
  let rec loop index rest = (
    let this = Primitives.FilledRect (
      {x = 0 ; y = index * band_height},
      {x = w ; y = (index + 1) * band_height},
      if ((sin ft) *. (sin ((Float.of_int index) /. (Random.float 10.))) > 0.5) then over_col else base_col) in
    match index with
    | 0 -> this :: rest
...
let tick t s _p _i =
  ...
  for iy = 0 to (rows - 1) do
    for ix = 0 to (cols - 1) do
      ...
      and y = iy * ystep in
      Random.init (x + y);
      let tile = generate_tile ((Float.of_int t) /. 100.) (w/cols) (h/rows) (iy) ix in
